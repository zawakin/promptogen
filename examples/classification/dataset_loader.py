...
class DatasetLoader:
    def __init__(self, seed: int, train_size: int, test_size: int, dataset: DatasetDict, attributes: DatasetAttributes, label_map: Dict[int, str]):
        self.seed = seed
    ...
    def load_dataset(self, mode: str, size: int) -> List[pg.Example]:
        ...
        examples=[
            pg.Example(input={self.attributes.input_key: text}, output={self.attributes.output_key: self.label_map[label]})
...
class IMDbSentimentDataset(DatasetLoader):
    def __init__(self, seed: int = 0, train_size: int = 4, test_size: int = 50):
        attributes = DatasetAttributes(
            ...
            description="classify the sentiment of a movie review, as positive or negative; (review: str) -> (reasoning: str, sentiment: str)",
            background='sentiment should be "positive" or "negative" depending on the review. add examples for both positive and negative reviews.',
            name='imdb_sentiment'
        )
        label_map = {
            0: 'negative',
